...
class DashboardController extends Controller
{
    ...
    public function dashboard_details(Request $request)
    {
       ...
       if(isset($request->form_type) && $request->form_type ==  'search_dashboard') {
        ...
        #Total Number Users
        $SQL14 = 'SELECT Count(id) as TotalUserCount FROM users WHERE is_active = 1 AND is_deleted = 0';  
        $TotalNumberUsers = DB::select($SQL14,array());
        $aReturn['TotalNumberUsers'] = (count($TotalNumberUsers) > 0) ? $TotalNumberUsers[0]->TotalUserCount : 0;
        // dd(   $aReturn['TotalNumberUsers'] );

        #Remitted Amount
        $SQL14 = 'SELECT sum(amount_remitted) as amount_remitted FROM remittance_management WHERE active = 1';  
        $TotalRemittedAmount1 = DB::select($SQL14,array());
       ...
       #Category Booking Data bar chart table details 
        $SQL1 = "SELECT b.ticket_id,b.event_id,e.booking_date,SUM(b.quantity) AS TicketCount,(SELECT ticket_name FROM event_tickets WHERE id=b.ticket_id) AS TicketName,(SELECT ticket_price FROM event_tickets WHERE id=b.ticket_id) AS TicketPrice
        FROM booking_details AS b
        LEFT JOIN event_booking AS e ON b.booking_id = e.id
        WHERE e.transaction_status IN (1,3)";
        if ($aReturn['search_filter'] !== "") {
        ...
        $BookingData = DB::select($SQL1,array());
