...
class UserController extends Controller
{
    ...
    public function edit_user_medical(Request $request, $id = null)
    {
        $responseData = [];
        $response['message'] = "";
        $responseCode = 400;
        $empty = true; // Setting it to true by default
        ...
        if ($aToken['code'] == 200) {
            ...
            // Assuming $aToken['data']->ID is the user ID
            $userId = $aToken['data']->ID;
            // dd($userId);
            ...
            if ($userData) {
                $empty = false; // User details found, not empty
            }
            $Exist = array();
            if (!$empty) {
                ...
                $diabetes = (!empty($request->diabetes)) ? $request->diabetes : '';
                $chestpain = (!empty($request->chestpain)) ? $request->chestpain : '';
                $dehydrationseverity = (!empty($request->dehydrationseverity)) ? $request->dehydrationseverity : '';
                $musclecramps = (!empty($request->musclecramps)) ? $request->musclecramps : '';
                $lowbloodsugar = (!empty($request->lowbloodsugar)) ? $request->lowbloodsugar : '';
                $undermedication = (!empty($request->undermedication)) ? $request->undermedication : '';
                $drugallergy = (!empty($request->drugallergy)) ? $request->drugallergy : '';
                $angina = (!empty($request->angina)) ? $request->angina : '';
                $abnormalheartrhythm = (!empty($request->abnormalheartrhythm)) ? $request->abnormalheartrhythm : '';
                $pacemaker = (!empty($request->pacemaker)) ? $request->pacemaker : '';
                $highbloodpressure = (!empty($request->highbloodpressure)) ? $request->highbloodpressure : '';

                $epilepsy = (!empty($request->epilepsy)) ? $request->epilepsy : '';
                $bleedingdisorders = (!empty($request->bleedingdisorders)) ? $request->bleedingdisorders : '';
                $asthma = (!empty($request->asthma)) ? $request->asthma : '';
                $anemia = (!empty($request->anemia)) ? $request->anemia : '';
                $hospitalized = (!empty($request->hospitalized)) ? $request->hospitalized : '';
                $infections = (!empty($request->infections)) ? $request->infections : '';
                $pregnant = (!empty($request->pregnant)) ? $request->pregnant : '';

                $covidstatus = isset($request->covidstatus) ? $request->covidstatus : "";

                $currentmedications = isset($request->currentmedications) ? $request->currentmedications : "";
                ...
                user_id=:user_id WHERE id = :id';
                    ...
                    //  dd($Binding);
                    $rResult = DB::update($sql, $Binding);
                    //  dd($rResult);
                    // $ResponseData['result'] = $rResult;
                    // $ResposneCode = 200;
                    ...
                    $isql = 'INSERT INTO user_details( diabetes , chestpain, dehydrationseverity, musclecramps, lowbloodsugar,undermedication, drugallergy,angina,abnormalheartrhythm,pacemaker,highbloodpressure,epilepsy,bleedingdisorders,asthma,anemia,hospitalized,infections,pregnant,covidstatus,currentmedications,familydoctorname,familydoctorcontactno,user_id) VALUES
                 (:diabetes,:chestpain,:dehydrationseverity,:musclecramps,:lowbloodsugar,:undermedication,:drugallergy,:angina,:abnormalheartrhythm,:pacemaker,:highbloodpressure,:epilepsy,:bleedingdisorders,:asthma,:anemia,:hospitalized,:infections,:pregnant,:covidstatus,:currentmedications,:familydoctorname,:familydoctorcontactno,:user_id)';
...
//  
        ...
        if ($aToken['code'] == 200) {
            ...
            $removedata = DB::delete($sql, $Binding);
            if ($removedata) {
                $responseCode = 200;
